    ...
    public void onReceive(Context context, Bundle bundle) {
        ...
        try {
            ...
            if (payload.has("type")) {
                ...
                if(payload.has("sub_type")){
                    String subtype = payload.getString("sub_type");
                    if(subtype.equals("unsentMessages")) {
                        JSONObject subpayload = payload.getJSONObject("payload");
                        if(!subpayload.getString("status").equals("blocked")) {
                            DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
                            db.updateChat(subpayload.getString("status"), subpayload.getString("uniqueid"));
                        }
                    }
                    if(subtype.equals("unsentGroupMessages")) {
                        JSONObject subpayload = payload.getJSONObject("payload");
                        if(!subpayload.optString("group_unique_id").equals("")){
                            DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
                            db.updateGroupChatStatus(subpayload.getString("unique_id"), "sent");
                            if(MainActivity.isVisible) {
                                MainActivity.mainActivity.updateGroupUIChat(subpayload.getString("unique_id"));
                            }
                        }
                    }
                    if(subtype.equals("unsentChatMessageStatus")) {
                        JSONObject subpayload = payload.getJSONObject("payload");
                        DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
                        db.resetSpecificChatHistorySync(subpayload.getString("uniqueid"));
                        db = new DatabaseHandler(ctx.getApplicationContext());
                        db.updateChat(subpayload.getString("status"), subpayload.getString("uniqueid"));
                    }
                    if(subtype.equals("unsentGroupChatMessageStatus")) {
                        JSONObject subpayload = payload.getJSONObject("payload");
                        if(subpayload.has("status")) {
                            String demo = subpayload.getString("uniqueid");
                            DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
                            db.resetSpecificGroupChatHistorySync(demo);
                            db = new DatabaseHandler(ctx.getApplicationContext());
                            db.updateGroupChatStatus(subpayload.getString("status"), demo);
                        }
                    }
                    if(subtype.equals("unsentGroups")) {
                        JSONObject subpayload = payload.getJSONObject("payload");
                        DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
                        db.deleteGroupServerPending(subpayload.getString("unique_id"));
                    }
                    ...
                    if(subtype.equals("statusOfSentMessages")) {
                        JSONArray subpayload = payload.getJSONArray("payload");
                    ...
                    if(subtype.equals("statusOfSentGroupMessages")) {
                        JSONArray subpayload = payload.getJSONArray("payload");
                        for (int i=0; i < subpayload.length(); i++) {
                            JSONObject row = subpayload.getJSONObject(i);
                            DatabaseHandler db = new DatabaseHandler(ctx.getApplicationContext());
